...
title: NxDB 8.0.0
slug: 8.0.0.html
description: NxDB Major Release 8.0.0
---

# 8.0.0

When I created NxDB, two years ago, there where some things that I did not consider and other things that I just decided wrong.
With the breaking version `8.0.0` I rewrote some parts of NxDB and changed the API a bit. The focus laid on **better defaults** and **better performance**.
...
To solve this waste, I spend one month creating [a module that polyfills the broadcast-channel-api](https://github.com/nxpkg/broadcast-channel) so it works on old browsers, new browsers and even NodeJs. This does not only waste less resources but also has a lower latency. Also the module is used by more projects than NxDB which allows use the fix bugs and improve performance together.
