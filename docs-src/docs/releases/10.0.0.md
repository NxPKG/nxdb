...
After doing many tests and observations, I found out that the in-memory plugin was slow. Really slow, even slower then just using the indexeddb adapter in the browser. You can reproduce my observations at the event-reduce testpage. Here you can see that random-writes+query are slower on the [memory-adapter](https://nxdb.nxpkg.github.io/event-reduce/?tech=pouchdb:memory) then on [indexeddb](https://nxdb.nxpkg.github.io/event-reduce/?tech=pouchdb:indexeddb).
