...
The release [15.0.0](https://nxpkg.github.io/nxdb/releases/15.0.0.html) is used for major refactorings in the migration plugins and performance improvements of the RxStorage implementations.
...
The NxDB test suite now also runs in the [deno](https://deno.com/) runtime. Also there is a [DenoKV](https://nxpkg.github.io/nxdb/rx-storage-denokv.html) based RxStorage to use with Deno Deploy.
...
- FIX [#5311](https://github.com/nxpkg/nxdb/issues/5311) URL.createObjectURL is not a function in a browser plugin environment(background.js)
