...
# [NxDB](https://nxpkg.github.io/) 12.0.0
...
With the [cleanup plugin](https://nxpkg.github.io/cleanup.html), NxDB will run cleanup cycles that clean up deleted documents when it can be done safely.
...
If you have any collection with a custom index set, you need to run a [migration](https://nxpkg.github.io/migration-schema.html) when updating to NxDB version `12.0.0` so that NxDB can rebuild the indexes.
...
The [Memory RxStorage](https://nxpkg.github.io/rx-storage-memory.html) is based on plain in-memory arrays and objects. It can be used in all environments and is made for performance.
...
- [RxStorage IndexedDB](https://nxpkg.github.io/rx-storage-indexeddb.html) a really fast [RxStorage](https://nxpkg.github.io/rx-storage.html) implementation based on **IndexedDB**. Made to be used in browsers.
- [RxStorage SQLite](https://nxpkg.github.io/rx-storage-sqlite.html) a really fast [RxStorage](https://nxpkg.github.io/rx-storage.html) implementation based on **SQLite**. Made to be used on **Node.js**, **Electron**, **React Native**, **Cordova** or **Capacitor**.
- [RxStorage Sharding](https://nxpkg.github.io/rx-storage-sharding.html) a wrapper around any other [RxStorage](https://nxpkg.github.io/rx-storage.html) that improves performance by applying the sharding technique.
