...
# [NxDB](https://nxdb.khulnasoft.com/) 12.0.0
...
With the [cleanup plugin](https://nxdb.khulnasoft.com/cleanup.html), NxDB will run cleanup cycles that clean up deleted documents when it can be done safely.
...
If you have any collection with a custom index set, you need to run a [migration](https://nxdb.khulnasoft.com/migration-schema.html) when updating to NxDB version `12.0.0` so that NxDB can rebuild the indexes.
...
The [Memory RxStorage](https://nxdb.khulnasoft.com/rx-storage-memory.html) is based on plain in-memory arrays and objects. It can be used in all environments and is made for performance.
...
- [RxStorage IndexedDB](https://nxdb.khulnasoft.com/rx-storage-indexeddb.html) a really fast [RxStorage](https://nxdb.khulnasoft.com/rx-storage.html) implementation based on **IndexedDB**. Made to be used in browsers.
- [RxStorage SQLite](https://nxdb.khulnasoft.com/rx-storage-sqlite.html) a really fast [RxStorage](https://nxdb.khulnasoft.com/rx-storage.html) implementation based on **SQLite**. Made to be used on **Node.js**, **Electron**, **React Native**, **Cordova** or **Capacitor**.
- [RxStorage Sharding](https://nxdb.khulnasoft.com/rx-storage-sharding.html) a wrapper around any other [RxStorage](https://nxdb.khulnasoft.com/rx-storage.html) that improves performance by applying the sharding technique.
