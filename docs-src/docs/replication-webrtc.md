...
const replicationPool = await replicateWebRTC(
    {
        ...
        connectionHandlerCreator: getConnectionHandlerSimplePeer({
            ...
            signalingServerUrl: 'wss://signaling.nxdb.khulnasoft.com/',
...
A **custom conflict handler** can be set up, which is a plain JavaScript function. The conflict handler is run on each replicated document write and resolves the conflict if required. [Find out more about NxDB conflict handling here](https://nxdb.khulnasoft.com/transactions-conflicts-revisions.html)
