...
The NxDB Server Plugin makes it possible to spawn a server on top of a NxDB database that offers multiple types of endpoints for various usages. It can spawn basic CRUD REST endpoints or even realtime replication endpoints that can be used by the client devices to replicate data. The RxServer plugin is designed to be used in Node.js but you can also use it in Deno, Bun or the [Electron](./electron-database.md) "main" process. You can use it either as a **standalone server** or add it on top of an **existing http server** (like express) in nodejs.

:::warning beta
The server plugin is in **beta** mode and the API might be changed without a major NxDB release.
:::

## Starting a RxServer

To create an `RxServer`, you have to install the `nxdb-server` package with `npm install nxdb-server --save` and then you can import the `createRxServer()` function and create a server on a given [RxDatabase](./rx-database.md) and adapter.

After adding the endpoints to the server, do not forget to call `myServer.start()` to start the actually http-server.

```ts
import { createRxServer } from 'nxdb-server/plugins/server';

/**
 * We use the express adapter which is the one that comes with NxDB core
 */
import { RxServerAdapterExpress } from 'nxdb-server/plugins/adapter-express';
...
There is also a [NxDB Premium 👑](/premium) adapter to use the RxServer with [Fastify](https://fastify.dev/) instead of express. Fastify has shown to have better performance and in general is more modern.

```ts
import { createRxServer } from 'nxdb-server/plugins/server';
import { RxServerAdapterFastify } from 'nxdb-premium/plugins/server-adapter-fastify';
...
There is also a [NxDB Premium 👑](/premium) adapter to use the RxServer with [Koa](https://koajs.com/) instead of express. Koa has shown to have better compared to express.

```ts
import { createRxServer } from 'nxdb-server/plugins/server';
import { RxServerAdapterKoa } from 'nxdb-premium/plugins/server-adapter-koa';
...
The replication endpoint allows clients that connect to it to replicate data with the server via the NxDB [replication protocol](./replication.md). There is also the [Replication Server](./replication-server.md) plugin that is used on the client side to connect to the endpoint.
...
The REST endpoint exposes various methods to access the data from the RxServer with non-NxDB tools via plain HTTP operations. You can use it to connect apps that are programmed in different programming languages than JavaScript or to access data from other third party tools.
...
import { createRestClient } from 'nxdb-server/plugins/client-rest';
...
Normal NxDB schema indexes get the `_deleted` field prepended because all [RxQueries](./rx-query.md) automatically only search for documents with `_deleted=false`.
When you use NxDB on a server, this might not be optimal because there can be the need to query for documents where the value of `_deleted` does not mather. Mostly this is required in the [pull.stream$](./replication.md#checkpoint-iteration) of a replication when a [queryModifier](#query-modifier) is used to add an additional field to the query.
