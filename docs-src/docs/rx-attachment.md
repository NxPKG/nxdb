...
Internally, attachments in NxDB are stored and handled similar to how [CouchDB, PouchDB](https://pouchdb.com/guides/attachments.html#how-attachments-are-stored) does it.
...
import { NxDBAttachmentsPlugin } from 'nxdb/plugins/attachments';
...
The attachments of NxDB are represented by the type `RxAttachment` which has the following attributes/methods.
...
:::note
The digest is NOT calculated by NxDB, instead it is calculated by the RxStorage. The only guarantee is that the digest will change when the attachments data changes.
:::
...
} from 'nxdb/plugins/attachments-compression';
