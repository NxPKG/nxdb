...
Because NxDB is used in client side applications, it cannot do any background checks or measurements to optimize the query plan because that would cause significant performance problems.

:::note
The query optimizer is part of the [NxDB Premium 👑](/premium) plugin that must be purchased. It is not part of the default NxDB module.
:::
...
import {
    findBestIndex
} from 'nxdb-premium/plugins/query-optimizer';
