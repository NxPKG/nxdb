...
# NxDB Database Replication Protocol

The NxDB replication protocol provides the ability to replicate the database state in **realtime** between the clients and the server.

The backend server does not have to be a NxDB instance; you can build a replication with **any infrastructure**.
...
The replication is made to support the [Offline-First](./offline-first.md) paradigm, so that when the client goes offline, the NxDB database can still read and write locally and will continue the replication when the client goes online again.
...
If your backend for any reason is not able to provide a full `pullStream$` that contains all events and the checkpoint, you can instead only emit `RESYNC` events that tell NxDB that anything unknown has changed on the server and it should run the pull replication via [checkpoint iteration](#checkpoint-iteration).
...
Then your data is always sortable by `updatedAt`. This ensures that when NxDB fetches 'new' changes via `pullHandler()`, it can send the latest `updatedAt+id` checkpoint to the remote endpoint and then receive all newer documents.
...
**NxDB resolves all conflicts on the client** so it would call the conflict handler of the `RxCollection` and create a new document state `D` that can then be written to the master.
...
const replicationState = await replicateRxCollection({
     ...
     * and so that NxDB is able to resume the replication on app reload.
    ...
    retryTime: 5 * 1000,
    /**
     * When multiInstance is true, like when you use NxDB in multiple browser tabs,
     ...
     * NxDB will still store the documents internally with '_deleted', setting this field
    ...
    pull: {
        ...
        async handler(lastCheckpoint, batchSize) {
            ...
            return {
                 ...
                 * then NxDB assumes that there are no more un-replicated documents
         ...
         * but before they are used by NxDB.
         * Notice that the modifier can be called multiple times and should not contain any side effects.
         * (optional)
         */
        modifier: d => d,
...
For better performance, the replication runs only in one instance when NxDB is used in multiple browser tabs or Node.js processes.
...
you need to import the [leader election plugin](./leader-election.md) so that NxDB can know how many instances exist and which browser tab should run the replication.

## Error handling

When sending a document to the remote fails for any reason, NxDB will send it again in a later point in time.
...
A common mistake in NxDB usage is when developers want to block the app usage until the application is in sync.
...
The same can be done for the other direction by setting a `pull.initialCheckpoint`. Notice that here we need the remote checkpoint from the backend instead of the one from the NxDB storage.
...
Attachment replication is supported in the NxDB replication protocol itself. However not all replication plugins support it.
