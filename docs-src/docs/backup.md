...
This is useful to:
  - Consume the database content with other software that cannot replicate with NxDB
...
This means we have to import it to NxDB before it can be used.

```javascript
import { addRxPlugin } from 'nxdb';
import { NxDBBackupPlugin } from 'nxdb/plugins/backup';
addRxPlugin(NxDBBackupPlugin);
```
