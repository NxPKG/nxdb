...
The Filesystem Node [RxStorage](./rx-storage.md) for NxDB is built on top of the [Node.js Filesystem API](https://nodejs.org/api/fs.html).
...
- It is part of the [NxDB Premium 👑](/premium) plugin that must be purchased.
- It is in beta mode at the moment which means it can include breaking changes without a NxDB major version increment.
