...
RxState is a flexible state library build on top of the [NxDB Database](https://nxpkg.github.io/nxdb/). While NxDB stores similar documents inside of collections, RxState can store any complex JSON data without having a predefined schema.
...
A `RxState` instance is created on top of a [RxDatabase](./rx-database.md). The state will automatically be persisted with the [storage](./rx-storage.md) that was used when setting up the RxDatabase. To use it you first have to import the `NxDBStatePlugin` and add it to NxDB with `addRxPlugin()`.
...
// first add the RxState plugin to NxDB
import { NxDBStatePlugin } from 'nxdb/plugins/state';
addRxPlugin(NxDBStatePlugin);
...
```ts
import { RxReactivityFactory, createRxDatabase } from 'nxdb/plugins/core';
...
import { NxDBCleanupPlugin } from 'nxdb/plugins/cleanup';
addRxPlugin(NxDBCleanupPlugin);
```
...
RxState still uses NxDB which has a range of [great performing storages](./rx-storage-performance.md) so the write speed is more then sufficient. Also to further improve write performance you can use more RxState instances (with an different namespace) to split writes across multiple storage instances.

Reads happen directly in-memory which makes RxState read performance comparable to other state libraries.

## RxState Replication

Because the state data is stored inside of an internal [RxCollection](./rx-collection.md) you can easily use the [NxDB Replication](./replication.md) to sync data between users or devices of the same user.
...
- RxState is in beta mode, it might get breaking changes without having a major NxDB version release.
