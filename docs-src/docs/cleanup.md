...
To make the replication work, and for other reasons, NxDB has to keep deleted documents in the storage.
...
With the cleanup plugin, NxDB will run cleanup cycles that clean up deleted documents when it can be done safely.
...
const db = await createRxDatabase({
  ...
  cleanupPolicy: {
      ...
      /**
       * If set to true,
       * NxDB will await all running replications
       ...
       * This ensures that when NxDB is used in multiInstance mode,
