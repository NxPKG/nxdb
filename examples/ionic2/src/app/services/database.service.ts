...
NxDB.plugin(adapters[useAdapter]);

NxDB.plugin(require('pouchdb-adapter-http'));
...
export class DatabaseService {
  ...
  private async _create(): Promise<RxDatabase> {
    ...
    const db = await NxDB.create({ name: 'heroesdb', adapter: useAdapter, password: 'myLongAndStupidPassword' });
