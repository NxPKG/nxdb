...
console.log('syncURL: ' + environment.nxdbSyncUrl);
...
async function _create(injector: Injector): Promise<RxHeroesDatabase> {
     ...
     * @link https://nxdb.nxpkg.github.io/reactivity.html
     */
    ...
    if (doSync()) {
        ...
        await Promise.all(
            Object.values(db.collections).map(async (col) => {
                try {
                    ...
                    await fetch(
                        environment.nxdbSyncUrl + col.name + '/',
                        {
                            method: 'PUT'
                        }
                    );
        ...
        if (environment.isServerSideRendering) {
            ...
            const firstReplication = await replicateCouchDB({
                ...
                collection: db.hero,
                url: environment.nxdbSyncUrl + db.hero.name + '/',
                live: false,
                pull: {},
                push: {}
            });
