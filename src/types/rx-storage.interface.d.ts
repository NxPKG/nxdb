...
export interface RxStorage<Internals, InstanceCreationOptions> {
    ...
    /**
     * NxDB version is part of the storage
...
/**
 * User provided mango queries will be filled up by NxDB via normalizeMangoQuery()
...
export type FilledMangoQuery<RxDocType> = {
     ...
     * NxDB has to ensure that the primary key is always
...
> {
     ...
     * This is used by NxDB to known what has changed since X so these docs can be handled by the backup or the replication
     * plugin.
     *
     * Important: This method is optional. If not defined,
     * NxDB will manually run a query and use the last returned document
     * for checkpointing. In  the future we might even remove this method completely
     * and let NxDB do the work instead of the RxStorage.
     */
