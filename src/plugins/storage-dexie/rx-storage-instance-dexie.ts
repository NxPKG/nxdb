...
> {
    ...
    async bulkWrite(
        ...
        if (
            !shownNonPremiumLog &&
            (
                !NXDB_UTILS_GLOBAL.premium ||
                typeof NXDB_UTILS_GLOBAL.premium !== 'string' ||
                (await defaultHashSha256(NXDB_UTILS_GLOBAL.premium) !== PREMIUM_FLAG_HASH)
            )
        ) {
            console.warn(
                [
                    '-------------- NxDB Open Core RxStorage -------------------------------',
                    'You are using the free Dexie.js based RxStorage implementation from NxDB https://nxdb.nxpkg.github.io/rx-storage-dexie.html?console=dexie ',
                    'While this is a great option, we want to let you know that there are faster storage solutions available in our premium plugins.',
                    'For professional users and production environments, we highly recommend considering these premium options to enhance performance and reliability.',
                    ' https://nxdb.nxpkg.github.io/premium?console=dexie ',
