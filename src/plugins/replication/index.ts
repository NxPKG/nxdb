 ...
 * a NxDB client-server replication.
...
import { NxDBLeaderElectionPlugin } from '../leader-election/index.ts';
import {
    arrayFilterNotEmpty,
    ensureNotFalsy,
    errorToPlainJson,
...
export class RxReplicationState<RxDocType, CheckpointType> {
    ...
    public async start(): Promise<void> {
        ...
        this.internalReplicationState = replicateRxStorageInstance({
            ...
            identifier: 'nxdbreplication' + this.replicationIdentifier,
                ...
                masterWrite: async (
                    ...
                    while (!done && !this.isStopped()) {
                        try {
                             ...
                             * @link https://github.com/nxpkg/nxdb/issues/4103
                             */
                            ...
                            const emitError = (err as RxError).nxdb ? err : newRxError('RC_PUSH', {
                                pushRows: rows,
                                errors: toArray(err).map(er => errorToPlainJson(er)),
                                direction: 'push'
                            });
