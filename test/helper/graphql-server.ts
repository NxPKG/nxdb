...
export async function spawn(
    ...
    const schema = buildSchema(`
        ...
        type Query {
            info: Int
            feedForNxDBReplication(checkpoint: CheckpointInput, limit: Int!): FeedResponse!
            collectionFeedForNxDBReplication(checkpoint: CheckpointInput, limit: Int!): CollectionFeedResponse!
            getAll: [Human!]!
        }
        type Mutation {
    ...
    const root = {
        info: () => 1,
        collectionFeedForNxDBReplication: (args: any) => {
            const result = root.feedForNxDBReplication(args);
