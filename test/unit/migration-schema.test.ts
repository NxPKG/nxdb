...
import { NxDBAttachmentsPlugin } from '../../plugins/attachments/index.mjs';
...
describeParallel('migration-schema.test.ts', function () {
    ...
    describe('issues', () => {
        ...
        it('opening an older NxDB database state with a new major version should throw an error', async () => {
            const dbName = randomCouchString(10);
            const db = await createRxDatabase({
                name: dbName,
                storage: config.storage.getStorage(),
            ...
            newTokenDoc.data.nxdbVersion = '14.x.x';
