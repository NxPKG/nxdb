...
var CHANGES_COLLECTION_SUFFIX = exports.CHANGES_COLLECTION_SUFFIX = '-nxdb-changes';
var LOKI_BROADCAST_CHANNEL_MESSAGE_TYPE = exports.LOKI_BROADCAST_CHANNEL_MESSAGE_TYPE = 'nxdb-lokijs-remote-request';
var LOKI_KEY_OBJECT_BROADCAST_CHANNEL_MESSAGE_TYPE = exports.LOKI_KEY_OBJECT_BROADCAST_CHANNEL_MESSAGE_TYPE = 'nxdb-lokijs-remote-request-key-object';
var RX_STORAGE_NAME_LOKIJS = exports.RX_STORAGE_NAME_LOKIJS = 'lokijs';

/**
 * Loki attaches a $loki property to all data
 * which must be removed before returning the data back to NxDB.
 */
...
function getLokiDatabase(databaseName, databaseSettings) {
  return (0, _index.getFromMapOrCreate)(LOKI_DATABASE_STATE_BY_NAME, databaseName, () => {
    ...
    var databaseState = (async () => {
      ...
      {
        /**
         * NxDB uses its custom load and save handling
...
function getLokiSortComparator(_schema, query) {
  ...
  var fun = (a, b) => {
     ...
     * which is added by NxDB if not existing yet.
     */
    if (!compareResult) {
      throw (0, _rxError.newRxError)('SNH', {
        args: {
...
async function mustUseLocalState(instance) {
  if (instance.closed) {
    /**
     * If this happens, it means that NxDB made a call to an already closed storage instance.
     * This must never happen because when NxDB closes a collection or database,
