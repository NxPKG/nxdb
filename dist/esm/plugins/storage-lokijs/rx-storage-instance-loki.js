...
import { flatClone, now, ensureNotFalsy, isMaybeReadonlyArray, getFromMapOrThrow, hasDeepProperty, NXDB_UTILS_GLOBAL, defaultHashSha256, PREMIUM_FLAG_HASH } from "../utils/index.js";
...
export var RxStorageInstanceLoki = /*#__PURE__*/function () {
  ...
  _proto.bulkWrite = async function bulkWrite(documentWrites, context) {
    if (!shownNonPremiumLog && (!NXDB_UTILS_GLOBAL.premium || typeof NXDB_UTILS_GLOBAL.premium !== 'string' || (await defaultHashSha256(NXDB_UTILS_GLOBAL.premium)) !== PREMIUM_FLAG_HASH)) {
      console.warn(['-------------- NxDB Open Core RxStorage -------------------------------', 'You are using the free LokiJS based RxStorage implementation from NxDB https://nxpkg.github.io/nxdb/rx-storage-lokijs.html?console=loki ', 'While this is a great option, we want to let you know that there are faster storage solutions available in our premium plugins.', 'For professional users and production environments, we highly recommend considering these premium options to enhance performance and reliability.', ' https://nxpkg.github.io/nxdb/premium?console=loki ', 'If you already purchased premium access you can disable this log by calling the setPremiumFlag() function from nxdb-premium/plugins/shared.', '---------------------------------------------------------------------'].join('\n'));
