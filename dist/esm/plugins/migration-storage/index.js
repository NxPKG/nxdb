...
logFunction) {
  ...
  /**
   * In NxDB v15 we changed how the indexes are created.
   * Before (v14), the storage prepended the _deleted field
   * to all indexes.
   * In v15, NxDB will prepend the _deleted field BEFORE sending
   * it to the storage. Therefore we have to strip these fields
   * when crating v14 storage instances.
   */
  if (!oldStorage.nxdbVersion && schema.indexes) {
    schema = clone(schema);
  ...
  /**
   * In NxDB v15 we removed statics.prepareQuery()
  ...
  var _loop = async function () {
      ...
      try {
        var writeToOldResult = await oldStorageInstance.bulkWrite(writeToOldRows, 'migrate-between-nxdb-versions');
      ...
      await oldStorageInstance.cleanup(0).catch(() => {
        /**
         * Migration from NxDB v14 to v15 had problem running the cleanup()
         ...
         * @link https://github.com/nxpkg/nxdb/issues/5565
         * 
