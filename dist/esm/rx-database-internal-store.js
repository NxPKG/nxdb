...
export async function ensureStorageTokenDocumentExists(rxDatabase) {
  ...
  var docData = {
    ...
    data: {
      nxdbVersion: rxDatabase.nxdbVersion,
      token: storageToken,
      /**
  ...
  if (error.isError && isBulkWriteConflictError(error)) {
    ...
    if (!isDatabaseStateVersionCompatibleWithDatabaseCode(conflictError.documentInDb.data.nxdbVersion, rxDatabase.nxdbVersion)) {
      throw newRxError('DM5', {
        args: {
          database: rxDatabase.name,
          databaseStateVersion: conflictError.documentInDb.data.nxdbVersion,
          codeVersion: rxDatabase.nxdbVersion
        }
      });
    }
