...
export type RxDocumentData<T> = T & {
     ...
     * NxDB also assumes that no data is finally deleted.
...
export type RxAttachmentData = {
     ...
     * It is calculated by NxDB, and send to the storage.
     ...
     * @link https://github.com/nxpkg/nxdb/pull/4107
     */
    digest: string;
};
 ...
 * that are send from NxDB to the RxStorage implementation.
 */
 ...
 * NxDB anyway only need the array data and we can save performance
...
export type RxStorageCountResult = {
     ...
     * If this returns 'slow', NxDB will throw by default
